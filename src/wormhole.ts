...
// faint swirl and, as the five beacon nodes charge, blooms into a bright blue
// flower of light. Purely cosmetic; driven by beaconIntensity() (0..1).
...
export class Wormhole {
  ...
  constructor() {
    ...
    petalRing(this.petals, 12, U * 1.7, U * 0.06, 0x9fd8ff, 0.5);
    petalRing(this.petals2, 8, U * 2.3, U * 0.04, 0xb39cff, 0.32);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      g.stroke({ width: U * 0.05, color: 0x7fbaff, alpha: 0.26 });
      g.blendMode = "add";
    ...
    this.container.addChild(this.glow, this.swirl, this.petals2, this.petals, this.core);
  ...
  update(dt: number, intensity: number, cx: number, cy: number, baseR: number): void {
    ...
    this.container.scale.set((baseR / U) * (0.28 + ramp * 0.95)); // small swirl → full bloom
    this.container.alpha = 0.18 + ramp * 0.82; // faint → vivid
