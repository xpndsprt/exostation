...
- **Power conduits (M-conduit):** generators/batteries energize the grid **6 tiles** (Chebyshev); a non-broken **Power Conduit** (¢1/tile) relays it **+6 tiles** per run. A consumer the grid can't reach is unpowered (no gating when there are zero generators/batteries — that's a normal blackout). Conduits lose **0.25 hp/s** each (independent → longer networks break more often); at hp 0 a conduit **stops conducting** until crew repair it (**+15 hp/s**, same as machinery). No per-tick credit upkeep — the cost is build price + repair labour.
