...
- **Loop:** `docked → outbound → transit (off-map) → inbound → unload`. A drone only flies when its bay is **powered**, it has a target with richness left, **and the station has storage to unload into** (`hasStorage` — any Storage Floor tile or a Silo; else it stays docked). It auto-repeats trips until the body **depletes (finite)**, then un-targets and idles.
...
- **Founding charter subsidy (income floor):** **0.5¢/s** per *certified* resident — alive **and** breathing well (`o2 ≥ 50`) — counted up to **5 crew** (cap ≈ **2.5¢/s**). It's the opening's only income before lodging/trade: a small well-run station (≤2–3 modules, a couple of crew) nets slightly **positive** on it alone, so the player isn't bankrupt before building a Bot Bay / Hotel / Trade Hub. The crew cap makes it negligible mid/late game, and suffocating crew (o2 below 50) stop earning it, so it rewards working life support rather than being free money.
- Credits can't go below 0. A **net ¢/s** readout (≈20s-smoothed) sits on the credits chip, red when negative.
- Result: a **tiny** station roughly breaks even (or trickles up) on the subsidy; a **module-heavy idle** station still **bleeds** (upkeep tops the capped stipend); a typical active mid-game station nets roughly **+7–8¢/s**. The player manages a margin and must keep income running, but competent play profits.
...
- **Tier-gated storage (`storageBlocksBuild` in `storage.ts`):** advanced (**2+ Lab**) modules each **reserve `SLOTS_PER_ADVANCED = 2` warehouse slots** and **can't be placed** without free capacity. Slots: **Storage Floor tile = 1**, **Silo = 5**, **Cargo Exchange = 8** (`warehouseSlots`). Gated set = `WATER_MODULE_KINDS` (every 2+-Lab build tool) **minus `silo`/`cargoex`** (the storage modules are exempt, or the fix would gate itself); Storage Floor is a tile, also exempt. Tier-1 starters are never gated. Enforced in the build handler (alert + `build-invalid`) and reflected in the ghost (turns red); the Advisor warns once you run out. This makes storage a prerequisite for tier-2+ expansion, not optional.
- **Storage floor visuals (`drawStorage`):** the tiles render the *actual* warehouse contents — ore crates, meal crates and a water tank, each good getting a contiguous zone of tiles that fills/empties in proportion to the real stock (no longer arbitrary per-tile crates).
...
| Raider | a pirate ship parks at a dock for **18 s**; module DPS = **min(48, 16 + 0.7·poweredModules)** (scales with station value), ×**0.5** under Garrison doctrine, ×**0.35** on the **very first raid** (`w.raidCount ≤ 1`). Chews a random non-life-support module's condition; **at 0 condition the module is destroyed (erased)** with a toast + a red attack beam in the renderer. **Also blows open a hull wall every 40 ticks (~4 s)** during the raid, venting the wing to space (only on 2+ room stations — beginner grace). Targets **life support too** when **no Turret has ever been built**, the station has **2+ rooms**, and the doctrine isn't Garrison. Each powered **Turret** lasers the nearest raider for **75 HP/s** (raider hull **100 HP** → ~1.3 s to down it; turrets focus-fire and stack, so more turrets / fewer raiders = faster kills). The raider still arrives and can chip a module while alive |
- **Gentle first raid (`w.raidCount`):** the first raid ever is a probe — module DPS ×**0.35**, **1** boarder (vs 2–4), and **no hull breaching** (`breach` skipped while `raidCount ≤ 1`). The counter increments per raid in `raid()`; from the 2nd on, raiders hit at full strength.
