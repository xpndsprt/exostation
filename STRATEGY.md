...
| Lighting | the interior is **dark & moody** by default (cool ambient); powered **Light Fixtures** (¢30) and glowing modules throw **soft, smoothly-feathered warm pools** that **cast real shadows** (walls/large modules block light). Every crew member carries a **moving lamp** (~4 cells) whose shadow sweeps as they walk. Visual only — no gameplay effect |
...
| Lodging prep | every Crew Quarters / Hotel Room is **assigned to one species** (Select → Reassign species); only that species sleeps there. Lets you keep same-air rivals (Human/Korro) in separate cabins. A small species-colour chip marks each bunk |
