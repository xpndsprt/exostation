...
- **🔊 / 🔇** (top bar) toggles **sound** (state is remembered). Audio starts on your first click/keypress. A **shuffled soundtrack** (from `assets/music/`) plays under the SFX at ~30% volume.
...
2. A **free Crew Quarters prepped for that species** — every bunk is **assigned to one species** (Select it → **Reassign species**); only that species sleeps there. So Crew Quarters are your crew capacity, **per species**. **Human is free to prep; every other species needs its host research first** (e.g. Methane for Thol, Chlorine for Chlorithe, Robotics for Korro, Exobiology for Sszra).
...
- **Crew physically haul the goods now.** A Bio Vat's output **piles up in an on-vat buffer** (up to **9**); a **resident carries it to a Storage Floor** (or, until you build one, to any free cell in the vat's room). **If no one hauls it and the buffer fills, the vat stalls** — so growth needs both crew and somewhere to put the harvest. Crew also **fetch feedstock from storage to the Synth** when storage exists (you'll see them carrying a coloured crate). *Bootstrap:* with no resident crew yet, vat output trickles straight to the warehouse so the opening still works.
- **Storage Floor** (¢3/tile, **Build** tab): an **airless** deck — only a **Light Fixture** can sit on it — and it **raises your meal & mineral caps**. It's the warehouse crew haul to. **Mess Table** (¢50, 3×3, **Modules**): crew **and guests eat at the seats around it**; other crew keep it stocked by carrying meals from storage. Neither needs research. *(If no table is stocked, crew fall back to eating at the Synth, so nobody starves.)*
- **Takeaway:** build **at least one Bio Vat per Synth**, lay some **Storage Floor**, and keep enough **crew** to haul. If meals stall, check power, the vat's haul (is its buffer backed up?), and that there's storage to carry to.
...
- **Raider** — a hostile **pirate craft** (its own dark-red ship + a red attack beam to whatever it's hitting) parks at your dock and **chews through a module's condition — and once that hits 0 the module is DESTROYED** (gone, not just broken). **Damage scales with station size** (~8/s up to ~26/s), and an **undefended** established station (2+ rooms, **no Turret ever built**) even exposes **life support**. With nothing left to wreck, the raider **breaches the hull** instead. A **powered Turret** (unlock *Station Security*) shoots it down on sight before it lands a hit; the **Garrison Doctrine** halves raider damage and keeps life support off-limits. **No defense → you lose modules.**
...
| Mining | minerals only, via the Bot Bay **Star Chart**: a system of orbiting bodies around one or two **stars** — planets (some with **moons**), an asteroid belt, scattered rocks. Dispatch a drone (off-map). Round trip ~25s (near asteroid) → ~70s (far planet); haul = body yield (asteroid 5–18, ore-rich 30–55, planet 35–90, moon 10–30), ×1.5 Korro. First trip surveys + delivers; bodies are finite. **Each trip risks the drone (~2% near → ~8% far); a lost drone is rebuilt by its Bay for ¢300 / ~18s.** Storage idles at the minerals cap |
